...
export default function Donations() {
  ...
  // Open the add form if redirected from dashboard
  useEffect(() => {
    if (location.state?.openAddForm) {
      setOpenAdd(true);
      // Clear the state to prevent the form from reopening on refresh
      window.history.replaceState({}, document.title);
    }
  }, [location.state]);

  const { data: res, isLoading } = useQuery(['donations', { page: 1, limit: 50, type: 'one-time' }], () =>
    donationsAPI.getDonations({ page: 1, limit: 50, type: 'one-time' }).then(r => r.data)
  );
  ...
  const { data: campaignsRes } = useQuery(['campaigns', { page: 1, limit: 1000 }], () => campaignsAPI.getCampaigns({ page: 1, limit: 1000 }).then(r => r.data));
  ...
  const selOrg = watch('organization');
  const selChannel = watch('channel');
  const selAgency = watch('agency');
  const campaignOptions = campaigns.filter(c => {
    const matchOrg = selOrg ? ((c.organization?._id || c.organization) === selOrg) : true;
    const matchChannel = selChannel ? ((c.channel?._id || c.channel) === selChannel) : true;
    const matchAgency = selAgency ? ((c.agency?._id || c.agency) === selAgency) : true;
    return matchOrg && matchChannel && matchAgency;
  });

  const createMutation = useMutation((payload) => donationsAPI.createDonation(payload).then(r => r.data), {
    onSuccess: () => {
      toast.success('One-time donation created successfully');
  ...
  const selOrgEdit = watchEdit('organization');
  const selChannelEdit = watchEdit('channel');
  const selAgencyEdit = watchEdit('agency');
  const campaignOptionsEdit = campaigns.filter(c => {
    const matchOrg = selOrgEdit ? ((c.organization?._id || c.organization) === selOrgEdit) : true;
    const matchChannel = selChannelEdit ? ((c.channel?._id || c.channel) === selChannelEdit) : true;
    const matchAgency = selAgencyEdit ? ((c.agency?._id || c.agency) === selAgencyEdit) : true;
    return matchOrg && matchChannel && matchAgency;
  });

  const updateMutation = useMutation(({ id, data }) => donationsAPI.updateDonation(id, data).then(r => r.data), {
    onSuccess: () => {
      toast.success('One-time donation updated successfully');
      queryClient.invalidateQueries(['donations']);
      setOpenEdit(false);
  ...
  const exportMutation = useMutation(({ format, ids }) => donationsAPI.exportDonations({ format, ids: ids.join(',') }), {
    onSuccess: (response, vars) => {
      ...
      a.href = url; a.download = vars.format === 'pdf' ? 'one-time-donations.pdf' : 'one-time-donations.xlsx';
    ...
    <div>
      ...
      <Toolbar>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <Button onClick={() => setOpenAdd(true)}>
            <i className="fas fa-plus"></i> New One-time Donation
          </Button>
        </div>
      </Toolbar>
      ...
      {openExport && (
        <ModalBackdrop onClick={() => setOpenExport(false)}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            <h3>Export One-time Donations</h3>
